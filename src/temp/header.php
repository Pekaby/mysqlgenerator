<?
 require 'modules/abstract/db.php';
if (isset($_COOKIE['UNID']) && isset($_COOKIE['SSID']) && isset($_COOKIE['hash_e'])){
  $ssid = $_COOKIE['SSID'];
  $un_id = $_COOKIE['UNID'];
  $pswd = $_COOKIE['pswd'];

  $stmt = db::execute("SELECT `img`, `email_vac` FROM `users` WHERE `un_id` = r$un_id AND `ssid` = $ssid AND `password` = $pswd");
  $dcs = $stmt->fetch();
...
<head>
  ...
  <link rel="stylesheet" href="src/temp/css/tmp.css">
...
<body>
  <header class="header">
    <div class="cont">
      <div class="logo_dv">
        <a href="/" class="logo">MySQL Generator | Support</a>
      </div>
